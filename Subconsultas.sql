...
SELECT * FROM Clientes2015
WHERE EXISTS (
				SELECT Codigo FROM Clientes2016
...
SELECT F.NomeFuncionario
  FROM Funcionario AS F
 WHERE F.idMatricula NOT IN (
								SELECT D.idMatricula FROM Dependente D 
                            );

--Retornar funcionários com salário acima da média salarial
SELECT AVG(SALARIO) FROM Funcionario; /*Pegando todos os salarios dos funcionários, somando e dividindo pela quantidade de funcionários*/

SELECT NomeFuncionario, Admissao, Salario
  FROM Funcionario
